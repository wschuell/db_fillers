...
class Getter(object):
    ...
    def get(self, db, raw_result=False, **kwargs):
        ...
        if raw_result:
            ...
            df = pd.DataFrame(
                self.parse_results(query_result=query_result), columns=self.columns
            )
            return df
    ...
    def plot_result(self, show=True, outfile=None, plot_kwargs={}, **kwargs):
        df = self.get_result(**kwargs)
